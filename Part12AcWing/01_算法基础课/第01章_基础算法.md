...
+ `√`[786.第k个数](https://www.acwing.com/problem/content/788/)：快速选择法，时间复杂度O(n)
    ...
    class Main {
        ...
        static int quick_select(int l, int r, int k) {
            ...
            return quick_select(j + 1, r, k - sl); // 要找的第k小元素在右半边，因为左半边已经有sl个元素，所以在新的递归中要找第k-sl小元素
        }
        ...
        long merge_sort(int l, int r) {
            ...
            while (i <= mid && j <= r) {
                if (q[i] <= q[j]) tmp[k++] = q[i++];
                    ...
                    tmp[k++] = q[j++];
                    // 这一步是关键，前面的归并已经保证了q[mid...i]区间内的元素大于q[i]切且小于q[mid],
                    // 这些元素都会和q[j]组成逆序对
                    res += mid - i + 1;
                }
            }
...
+ 一维前缀和 
  ...
  + $a[l] + ... + a[r] =\sum_{i=l}^{r}a[i] = S[r] - S[l - 1]$
    ...
    public class Main {
        static final int N = 100010;  // 定义数组大小, 防止越界
...
+ 二维前缀和
  ...
  + 以(x1, y1)为左上角，(x2, y2)为右下角的子矩阵的和为：$S[x2][y2] - S[x1 - 1][y2] - S[x2][y1 - 1] + S[x1 - 1][y1 - 1]$
    ...
    public class Main {
        static final int N = 1010; // 因为下标从1开始算，所以不要去正好取1000，再大了会Segmentation Fault
        static int n, m, q;
        static int[][] a = new int[N][N]; // a[i][j]表示数组元素;
        static int[][] S = new int[N][N]; // S[i][j]表示(1,1)这个点与(i,j)这个点两个点分别为左上角和右下角所组成的矩阵内的数的和
...
+ 二维差分
  ...
  + 更新a[i]: `a[i][j] = a[i-1][j] + a[i][j-1] - a[i-1][j-1] + p[i][j];`
    ...
    public class Main {
        ...
        static int n, m, q;
        static int[][] a = new int[N][N]; // a[i][j]表示数组元素;
        static int[][] p = new int[N][N]; // p[N]表示差分数组


        public static void main(String[] args) throws IOException {
            // 初始化输入值
            BufferedReader bf = new BufferedReader(new InputStreamReader(System.in));
            String[] nmq = bf.readLine().trim().split(" ");
            n = Integer.parseInt(nmq[0]);
            m = Integer.parseInt(nmq[1]);
            q = Integer.parseInt(nmq[2]);
            // 读取a并计算p
            for (int i = 1; i <= n; i++) {
                String[] aim = bf.readLine().trim().split(" ");
                for (int j = 1; j <= m; j++) {
                    a[i][j] = Integer.parseInt(aim[j - 1]);
                    p[i][j] = a[i][j] - a[i][j - 1] - a[i - 1][j] + a[i - 1][j - 1];
                }
            }
            while (q-- > 0) {
                int x1, y1, x2, y2, c;
                String[] x1y1x2y2c = bf.readLine().trim().split(" ");
                x1 = Integer.parseInt(x1y1x2y2c[0]);
                y1 = Integer.parseInt(x1y1x2y2c[1]);
                x2 = Integer.parseInt(x1y1x2y2c[2]);
                y2 = Integer.parseInt(x1y1x2y2c[3]);
                c = Integer.parseInt(x1y1x2y2c[4]);
                p[x1][y1] += c;
                p[x2 + 1][y1] -= c;
                p[x1][y2 + 1] -= c;
                p[x2 + 1][y2 + 1] += c;
