...
## 四、`前缀和`与`差分`
### 1.前缀和
> 参考：https://blog.csdn.net/qq_41661809/article/details/86727017

对于一个给定的数列 a, 它的前缀和数列S 是通过递推能求出来得 $S[i]=\sum_{j=1}^{i}a[j]$

部分和,即数列a中某个下标区间内的数的和,可以表示为前缀和相减的形式:$sum[l, r]=\sum_{i=l}^{r}a[i]=S[r]-S[l-1]$
