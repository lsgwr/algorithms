...
public class Main {
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            if (i >= k) System.out.print(a[qAsc.getFirst()] + " "); // 队列头元素就是最小元素 .i >= k是为了初始时先让滑动窗口挤满k个元素，然后再求滑动窗口内的最小值才有意义
        }
        System.out.println();
        ...
        for (int i = 1; i <= n; i++) {
            ...
            if (i >= k) System.out.print(a[qDsc.getFirst()] + " "); // 队列头元素就是最大元素 .i >= k是为了初始时先让滑动窗口挤满k个元素，然后再求滑动窗口内的最大值才有意义
        }
    }
}
```
