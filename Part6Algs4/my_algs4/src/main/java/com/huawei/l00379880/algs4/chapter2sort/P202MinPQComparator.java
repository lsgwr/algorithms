...
/******************************************************************************
 * @Description : P198~P203 基于堆的最小优先队列  !!!仅适用于自定义Comparator!!!
 *                (通过上浮和下沉形成完全二叉堆)
 *                Key 必须是自定义比较规则地.
 ...
 *****************************************************************************/
public class P202MinPQComparator<Key> implements Iterable<Key> {
    /**
     ...
     */
    public P202MinPQComparator(Comparator<Key> comparator, int initCapacity) {
    ...
    public P202MinPQComparator(Comparator<Key> comparator) {
        // 啥都不传就初始化为1的优先队列数组
        this(comparator, 1);
    }


    public P202MinPQComparator(Comparator<Key> comparator, Key[] keys) {
        this.comparator = comparator;
        n = keys.length;
    ...
    public Key delMin() {
        ...
        // 排好地二叉堆,最小元素一定是最上面的元素
        Key min = pq[1];
        // 把最上面的顶点和最下面的节点交换
        P153CommenFuns.exchange(pq, 1, n--);
        // 把交换上来的元素沉到合适位置
        sink(1);
        // 把交换下来的最小元素所在的位置的元素释放掉,帮助JVM回收.
    ...
    private void sink(int k) {
        while (2 * k <= n) {
            ...
            // 下面的k处的元素2k、2k+1中的元素较小者进行交换.
            // 之所以可能pq[2k]<pq[zk+1],是因为最小堆的
            // 要求只是顶点小于左右节点即可.左右节点的大小关系
            // 随便.自己一直与之混淆的是二叉查找树:其顶点大于
            // 左节点而小于右节点.
            if (j < n && P153CommenFuns.more(comparator, pq[j], pq[j + 1])) {
                // 下面的k处的元素2k、2k+1中的元素较大者进行交换
                j++;
            }
            // 如果pq[k]满足小于左右两个节点,那么下沉完成,退出while循环
            if (P153CommenFuns.less(comparator, pq[k], pq[j])) {
                break;
            }
    ...
    private void swim(int k) {
        while (k > 1 && P153CommenFuns.less(comparator, pq[k], pq[k / 2])) {
    ...
    private boolean isMinHeap(int k) {
        ...
        int left = 2 * k;
        int right = 2 * k + 1;
        // 任何一个地方的顶点如果大于左节点或者右节点,那么一定不是最小堆
        if (left <= n && P153CommenFuns.more(comparator, pq[k], pq[left])) {
            return false;
        }
        if (right <= n && P153CommenFuns.more(comparator, pq[k], pq[right])) {
            return false;
        }
    ...
    private class HeapIterator implements Iterator<Key> {
        ...
        /**
         * create a new pq,为了在遍历的时候不影响原最小堆
         */
        private P202MinPQComparator<Key> pqCopy;

        /**
         * 把所有元素复制到pqCopy数组中
         */
        public HeapIterator() {
            pqCopy = new P202MinPQComparator<>(comparator, size());
            for (int i = 1; i <= n; i++) {
                pqCopy.insert(pq[i]);
            }
        }
