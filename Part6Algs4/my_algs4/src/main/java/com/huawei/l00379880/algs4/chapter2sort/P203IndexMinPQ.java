 ...
 ***********************************************************/
public class P203IndexMinPQ<Key extends Comparable<Key>> implements Iterable<Integer> {
    /**
     * 优先队列的最大元素数目
     */
    private int maxN;
    ...
    public P203IndexMinPQ(int maxN) {
        if (maxN < 0) {
    ...
    public void increaseKey(int i, Key key) {
        ...
        if (!P153CommenFuns.more(key, keys[i])) {
    ...
    public void decreaseKey(int i, Key key) {
        ...
        if (!P153CommenFuns.less(key, keys[i])) {
            throw new IllegalArgumentException("新的Key必须要比老的Key大!");
        }
        keys[i] = key;
        //来更小的元素就该上浮,注意这是最小堆
    ...
    private void swim(int k) {
        while (k > 1 && P153CommenFuns.less(keys[pq[k]], keys[pq[k / 2]])) {
            exchange(k, k / 2);
            k = k / 2;
        }
    }
    ...
    private void sink(int k) {
        while (2 * k <= n) {
            int j = 2 * k;
            if (j < n && P153CommenFuns.more(keys[pq[j]], keys[pq[j + 1]])) {
                j++;
            }
            // 当父节点小于左右节点较小者时,堆有序了,退出while循环
            if (P153CommenFuns.less(keys[pq[k]], keys[pq[j]])) {
                break;
            }
            exchange(k, j);
            // 没完地话就继续下一轮
    ...
    private class HeapIterator implements Iterator<Integer> {
         ...
         * 元素数组的拷贝
         */
        private P203IndexMinPQ<Key> keysCopy;

        public HeapIterator() {
            this.keysCopy = new P203IndexMinPQ<Key>(pq.length - 1);
