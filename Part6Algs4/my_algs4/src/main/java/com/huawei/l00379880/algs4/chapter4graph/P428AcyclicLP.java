/***********************************************************
 * @Description : 基于拓扑排序的最长路径计算方法，更快更普适
...
public class P428AcyclicLP {
    /**
     * distTo[v]:起点s到v的最长距离
     */
    private double[] distTo;
    /**
     * edgeTo[v]:起点s到v的最长路径上的最后一条边
     */
    private P415DirectedEdge[] edgeTo;

    public P428AcyclicLP(P415EdgeWeightedDigraph digraph, int s) {
        int V = digraph.getV();
        distTo = new double[V];
        edgeTo = new P415DirectedEdge[V];
        validateVertex(s);
        // 初始化每个点到起点的最大距离。初始化负无穷小
        for (int v = 0; v < V; v++) {
