 ...
 * @description
 */
public class P203IndexMinPQTest {
    @Test
    public void testComparable() {
        ...
        P203IndexMinPQ<DogComparble> pq = new P203IndexMinPQ<>(list.size());
    ...
    public void testMain() {
        ...
        P203IndexMinPQ<String> pq = new P203IndexMinPQ<>(strings.length);
        for (int i = 0; i < strings.length; i++) {
            pq.insert(i, strings[i]);
        }

        System.out.println("依次输出优先队列中的最小元素!");
        // print each key using the iterator
        // 优先队列中存的是keys中元素的下标,依次此最小、次小....最大的下标
        for (int i : pq) {
            // i为在数组中的下标
