 ...
 ***********************************************************/
public class P203IndexMaxPQ<Key extends Comparable<Key>> implements Iterable<Integer> {
    /**
     * 优先队列的最大元素数目
     */
    private int maxN;
    ...
    public P203IndexMaxPQ(int maxN) {
        if (maxN < 0) {
    ...
    private void swim(int k) {
        while (k > 1 && P153CommenFuns.more(keys[pq[k]], keys[pq[k / 2]])) {
            exchange(k, k / 2);
            k = k / 2;
        }
    }
    ...
    private void sink(int k) {
        while (2 * k <= n) {
            int j = 2 * k;
            if (j < n && P153CommenFuns.less(keys[pq[j]], keys[pq[j + 1]])) {
                j++;
            }
            // 当父节点大于左右节点较大者时,堆有序了,退出while循环
            if (P153CommenFuns.more(keys[pq[k]], keys[pq[j]])) {
                break;
            }
            exchange(k, j);
            // 没完地话就继续下一轮
    ...
    private class HeapIterator implements Iterator<Integer> {
         ...
         * 元素数组的拷贝
         */
        private P203IndexMaxPQ<Key> keysCopy;

        public HeapIterator() {
            this.keysCopy = new P203IndexMaxPQ<Key>(pq.length - 1);
