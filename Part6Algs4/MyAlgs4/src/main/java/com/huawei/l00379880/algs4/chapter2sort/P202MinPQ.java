...
/***********************************************************
 * @Description : P198~P203 基于堆的最小优先队列
 *                (通过上浮和下沉形成完全二叉堆)
 *                Key 必须是自定义比较规则地.
 ...
 ***********************************************************/
public class P202MinPQ<Key extends Comparable<Key>> implements Iterable<Key> {
    /**
     ...
     */
    public P202MinPQ(int initCapacity) {
        // 不能直接创建泛型数组.
    ...
    private void sink(int k) {
        while (2 * k <= n) {
            ...
            // 左节点而小于右节点.
            if (j < n && P153CommenFuns.more(pq[j], pq[j + 1])) {
                // 下面的k处的元素2k、2k+1中的元素较大者进行交换
                j++;
            }
            // 如果pq[k]满足小于左右两个节点,那么下沉完成,退出while循环
            if (P153CommenFuns.less(pq[k], pq[j])) {
                break;
            }
    ...
    private void swim(int k) {
        while (k > 1 && P153CommenFuns.less(pq[k], pq[k / 2])) {
    ...
    private boolean isMinHeap(int k) {
        ...
        if (left <= n && P153CommenFuns.more(pq[k], pq[left])) {
            return false;
        }
        if (right <= n && P153CommenFuns.more(pq[k], pq[right])) {
            return false;
        }
    ...
    private class HeapIterator implements Iterator<Key> {
         ...
         */
        private P202MinPQ<Key> pqCopy;

        /**
         * 把所有元素复制到pqCopy数组中
         */
        public HeapIterator() {
            pqCopy = new P202MinPQ<>(size());
            for (int i = 1; i <= n; i++) {
                pqCopy.insert(pq[i]);
            }
        }
