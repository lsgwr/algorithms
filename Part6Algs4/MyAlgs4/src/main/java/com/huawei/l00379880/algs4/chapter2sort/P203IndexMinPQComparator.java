...
/***********************************************************
 * @Description : 带索引的最小堆,pq、qp的详细讲解见
 *              https://www.cnblogs.com/nullzx/p/6624731.html
 *              在这个过程中keys数组是不会进行swim和sink地,因为pq和
 ...
 *
 *
 * @核心算法 通过比较keys中元素的大小关系, 把pq排成堆有序, 堆中的元素为"第x小"的x
 * @author      : 梁山广
 * @date        : 2018/1/3 21:07
 * @email       : liangshanguang2@gmail.com
 ***********************************************************/
public class P203IndexMinPQComparator<Key> implements Iterable<Integer> {
    /**
     * 优先队列的最大元素数目
     */
    private int maxN;
     ...
     * pq[i]代表在keys中第i小的元素在keys中的下标是pq[i]
     */
    private int[] pq;
    /**
     * 下标是从1开始
     * inverse of pq - qp[pq[i]] = pq[qp[i]] = i
     * qp[i]代表在keys中第i个元素在keys中是第pq[i]小
     */
    private int[] qp;
    /**
    ...
    public P203IndexMinPQComparator(Comparator<Key> comparator, int maxN) {
        if (maxN < 0) {
    ...
    public int delMin() {
        ...
        // pq[1]表示第一小的元素在keys中的下标
        int min = pq[1];
        // 在keys中的元素删除之前,先把pq和qp中的对应元素进行修改
        exchange(1, n--);
        // 修改keys中的元素
        sink(1);
        // 把数组最小元素置位null,相当于释放掉了.
        // keys中第下标为max的在keys中是第-1小,明显给初始化掉了.
        // 也是因为qp在一开始初始化的时候就初始化地-1
        qp[min] = -1;
        keys[min] = null;

        // 确保最小元素的下标在前面exchange(1,n--)中被移到了合适位置
        assert pq[n + 1] == min;
        // 把交换过去的最小元素在keys中的下标
        pq[n + 1] = -1;
        // 返回最小元素在数组中的下标
    ...
    public void delete(int i) {
        ...
        // 下标为i的元素在keys中是第qp[i]大,即第index小
        int index = qp[i];
        exchange(index, n--);
        swim(index);
        sink(index);
    ...
    /**
     * 如果当前节点小于父节点那么就上浮(针对pq和qp来进行地)
     */
    private void swim(int k) {
        while (k > 1 && P153CommenFuns.less(comparator, keys[pq[k]], keys[pq[k / 2]])) {
            exchange(k, k / 2);
            k = k / 2;
        }
    }
    ...
    /**
     * 如果当前节点大于子节点那么就下沉(针对pq和qp来进行地)
     */
    private void sink(int k) {
        while (2 * k <= n) {
            int j = 2 * k;
            if (j < n && P153CommenFuns.more(comparator, keys[pq[j]], keys[pq[j + 1]])) {
                j++;
            }
            // 当父节点小于左右节点较小者时,堆有序了,退出while循环
            if (P153CommenFuns.less(comparator, keys[pq[k]], keys[pq[j]])) {
                break;
            }
            exchange(k, j);
            // 没完地话就继续下一轮
    ...
    private class HeapIterator implements Iterator<Integer> {
         ...
         * 元素数组的拷贝
         */
        private P203IndexMinPQComparator<Key> keysCopy;

        public HeapIterator() {
            this.keysCopy = new P203IndexMinPQComparator<Key>(comparator, pq.length - 1);
            // 复制所有keys中的元素到keysCopy
            for (int i = 1; i <= n; i++) {
                // pq[i] = keys中第i小元素的下标
                // keys[pq[i]] = keys中第i小的元素
