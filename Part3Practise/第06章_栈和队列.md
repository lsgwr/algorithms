 ...
 ***********************************************************/
package Chapter06StackQueue.LeetCode20ValidParentheses;
...
class Solution {
    private class Node {
        ...
        public Node(String word, Node pre, int distance) {
            this.word = word;
            this.pre = pre;
            this.distance = distance;
        }
    ...
    /**
     * 判断单词word是否在node作为最后一个节点的链表上
     */
    private boolean inCurrentPath(Node node, String word) {
        while (node != null) {
    ...
    public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (String word : wordList) {
                ...
                boolean inCurrentPath = inCurrentPath(node, word);
                if (oneCharDiff(word, node.word)) {
                    if (visited.get(word) == null || !inCurrentPath) {
                        // 找到当前单词中所有的邻接单词(即只有一个不同字符的单词)
                        wordsMatch.add(word);
                    }
                }
                for (String wordMatch : wordsMatch) {
                    inCurrentPath = inCurrentPath(node, wordMatch);
                    Node nodeTmp = new Node(wordMatch, node, node.distance + 1);
                    // 一旦找到目标单词endWord，
                    if (wordMatch.equals(endWord)) {
                        if (!findMin || nodeTmp.distance <= minDistance) {
                            // 发现一个路径就加入进去
                            pathList.add(getPath(nodeTmp));
                            minDistance = nodeTmp.distance;
                            findMin = true;
                        }
                    } else if (!inCurrentPath) {
                        // 当前节点的路径小于
                        if (nodeTmp.distance <= minDistance) {
                            queue.add(nodeTmp);
                        }
                    }
                }
...
class Solution {
    ...
    public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (String wordMatch : wordsMatch) {
                boolean inCurrentPath = inCurrentPath(node, wordMatch);
                Node nodeTmp = new Node(wordMatch, node, node.distance + 1);
                // 一旦找到目标单词endWord，
                if (wordMatch.equals(endWord)) {
                    if (findMin && nodeTmp.distance > minDistance) {
