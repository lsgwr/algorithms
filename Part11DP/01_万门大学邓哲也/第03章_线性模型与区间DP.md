...
int main() {
    while (scanf("%s%s", a + 1, b + 1) != EOF) { // 只要有旧一直往下查
        int n = strlen(a + 1);
        memset(g, 0, sizeof(g));
        ...
        for (int len = 2; len <= n; len++) { //先求出把空串刷成目标串所需要的最少次数
            for (int l = 1; l <= n - len + 1; l++) {
                ...
                g[l][r] = 1 + g[l + 1][r];
                for (int k = l + 1; k <= r; k++) {
                    if (b[l] == b[k]) g[l][r] = min(g[l][r], g[l][k - 1] + g[k + 1][r]);
                }
            }
        }
        ...
        for (int i = 1; i <= n; i++) { // 再求出已有串刷成目标串的最少次数。
            ...
            if (a[i] == b[i]) f[i] = f[i - 1]; // 如果已有串与目标串在i处相等，则此处可以不用处理，这就是空串与已有串不同的地方
                ...
                for (int k = 1; k <= i; k++) f[i] = min(f[i], f[k - 1] + g[k][i]);
            }
        }
        printf("%d\n", f[n]);
    }
        ...
        while (sc.hasNext()) {
            ...
            for (int len = 2; len <= n; len++) {
                for (int l = 1; l <= n - len + 1; l++) {
                    ...
                    g[l][r] = g[l + 1][r] + 1;
                    for (int k = l + 1; k <= r; k++) {
                        if (b[l] == b[k]) g[l][r] = Math.min(g[l][r], g[l][k - 1] + g[k + 1][r]);
                    }
                }
            }
