...
> `dp[i]`表示目前考虑了第1个到第i个数，且子序列中最后一个数是a[i]的最长上升子序列长度
+ 阶段：前i个数
...
class Solution {
    public int lengthOfLIS(int[] a) {
        ...
        int[] dp = new int[N + 1];
        dp[0] = 1; // 只有一个元素的数组，最长上升子序列长度肯定为1
        int iMax = dp[0]; // 最短的长度至少为1
        for(int i = 1; i < N; i++) {
            int jMax = 0;
            for(int j = 0; j < i; j++) {
                if(a[j] < a[i]) jMax = Math.max(jMax, dp[j]); // 满足最长上升子序列要求
            }
            dp[i] = jMax + 1;
            iMax = Math.max(iMax, dp[i]); // 遍历所有的i找最长的上升子序列长度
        }
        return iMax;
    }
}
...
### 例题：[POJ3176](https://ac.nowcoder.com/acm/problem/107773)
> 题目大意：
...
public class Main {
    ...
    private static final int[][] dirs = {{0, 1}, {1, 0}, {-1, 0}, {0, -1}};
    private static int[][] grid;
    private static int[][] dp; // 存储中间的步骤值，减少递归次数
    private static boolean[][] visited;
    ...
    // 从位置(r, c)开始滑雪
    public static int dfs(int r, int c) {
        if (dp[r][c] > 0) return dp[r][c]; // 之前递归过了，就直接返回
        int result = 1; // 当前点开始的滑雪能滑地最大长度，至少当前点算一个，所以节点数为1
        visited[r][c] = true;
        // 遍历邻接点
        int max = 0; // 记录点(r, c)的邻接点中最长的滑雪长度
