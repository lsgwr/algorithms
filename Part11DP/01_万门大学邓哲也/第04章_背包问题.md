...
public class Main {
    static class Item implements Comparable<Item> {
        ...
        protected Object clone() throws CloneNotSupportedException {
            return new Item(t, d, p, num);
        }
    }

    static int n, ans, ansnum;
    static List<Integer>[] v = new List[2010]; // 列表数组，表示
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= n; i++) {
            for (int j = a[i].d - 1; j >= a[i].t; j--) { // 只能取一次，倒序循环（从a[i].d-1开始！）
                if (f[j - a[i].t] + a[i].p > f[j]) {
                    f[j] = f[j - a[i].t] + a[i].p; // 转移方程
                    v[j] = new ArrayList<>();
                    for (int e : v[j - a[i].t]) v[j].add(e); // 把v[j - a[i].t]赋值给v[j]
                    v[j].add(a[i].num); // 更新v[j]
                }
            }
        }
        for (int i = 0; i <= 2000; i++) {
        ...
        for (int i = 1; i < v[ansnum].size(); i++) System.out.print(v[ansnum].get(i) + " ");
    }
}
```
