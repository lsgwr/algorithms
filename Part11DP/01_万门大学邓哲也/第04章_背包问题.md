...
至此，问题也就转换成了第i个物体的体积是$c[i] = a[i] - k * b[i]$，价值是a[i]，的背包问题，为了防止总的体积被c[i]减成负的(`j-c[i]`要大于0)，所以初始化时j要足够大，我们取10000，这样我们就可以分成正负两个dp来做了

本问题即是求1~i对应的a和b放入后$\sum_{i=1}^{i}c[i]$是0，且$sum(a[i])=\sum_{i=1}^{i}a[i]$`即放入的所有的物体i的价值和`最大)
...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        c[i] = a[i] - k * b[i];
        if (c[i] >= 0) {
            // j初始化10000因为要保证 j≥c[i]的条件不成立前i要能循环完，因此j要足够大
            for (int j = 10000; j >= c[i]; j--) dp1[j] = max(dp1[j], dp1[j - c[i]] + a[i]);
        } else {
            // 处理c[i]小于0的情况
            c[i] = -c[i];
            for (int j = 10000; j >= c[i]; j--) dp2[j] = max(dp2[j], dp2[j - c[i]] + a[i]);
        }
    }
    int res = -1;
...
public class Main {
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            if (c[i] >= 0) {
                // j初始化10000因为要保证 j≥c[i]的条件不成立前i要能循环完，因此j要足够大
                for (int j = N; j >= c[i]; j--) dp1[j] = Math.max(dp1[j], dp1[j - c[i]] + a[i]);
            } else {
                // 处理c[i]小于0的情况
                c[i] = -c[i];
                for (int j = N; j >= c[i]; j--) dp2[j] = Math.max(dp2[j], dp2[j - c[i]] + a[i]);
            }
        }
        int res = -1;
        for (int i = 0; i <= N; i++) {
    ...
    while (~scanf("%d%d", &n, &m), n + m) {
        ...
        for (i = 1; i <= n; i++) {
            ...
            for (j = v[i]; j <= m; j++) { // 循环检查看是否能够出现前边没有出现的价格
                if (!dp[j] && dp[j - v[i]] && sum[j - v[i]] < c[i]) { // 如果j价格没有出现过，且j-v[i]出现过，并且使用i硬币的次数没有超出给定的数量
                    dp[j] = 1;
