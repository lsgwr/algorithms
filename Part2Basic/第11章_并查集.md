...
## 11.4 测试前面实现的UnionFind，并基于再次size优化UnionFind
### 测试并查集
> 主要是测试前面两节实现的两种UnionFind，当并查集的节点数size比union此时越大时，第3节基于树的并查集实现性能越高
+ [通用测试函数](src/main/java/Chapter11UnionFind/Section4TestUFAndOptimizeBySize/TestUF.java)
+ [测试类](src/main/java/Chapter11UnionFind/Section4TestUFAndOptimizeBySize/Main.java)
### 基于size优化进一步UnionFind